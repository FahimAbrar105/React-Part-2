...
const ChatRoom = () => {
    ...
    useEffect(() => {
        ...
        const fetchRoomData = async () => {
            try {
                ...
                // Refactor chatController.getChat to return JSON.
        ...
        const messageHandler = (msg: any) => {
            ...
            // Simplest check: sender is otherUser or sender is me and receiver is otherUser
            // The logic from ejs was: if (message.sender === receiver || message.sender === sender)
            // We need to verify if the message is for THIS chat context (user pair + product)
    ...
    const handleSendMessage = (e: React.FormEvent) => {
        ...
        // Optimistically add message? The socket listener above handles "message" event which server emits back.
        // If server broadcasts to sender too, we don't need to add it manually here.
        // Looking at backend chatController: 
        // socket.emit('message', ...) ->io.emit('message', formattedMessage)
        // It emits to everyone? No, it usually emits to specific room or users.
        // Assuming server implementation broadcasts back to sender as well (common in socket.io tutorials).
